  ...
  return (
    <div className={compact ? "space-y-3 rounded-2xl border border-zinc-800 bg-gradient-to-b from-zinc-900 to-zinc-950 p-3" : "space-y-4"}>
      <div className={`flex items-center justify-between gap-2 ${compact ? "flex-col items-start sm:flex-row sm:items-center" : ""}`}>
        <h3 className={compact ? "text-base font-extrabold text-zinc-100 tracking-tight" : "text-xl font-extrabold text-zinc-100"}>
          Monthly Deadlines
        </h3>
        <div className={`flex items-center gap-1.5 ${compact ? "rounded-lg border border-zinc-800 bg-zinc-900 px-1 py-1" : ""}`}>
          <button
            onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}
            className={`${compact ? "px-2 py-1 text-sm font-semibold" : "px-2.5 py-1.5 text-base font-semibold"} rounded-lg text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition`}
          >
            Prev
          </button>
          <span className={`${compact ? "text-sm min-w-[120px]" : "text-base min-w-[140px]"} font-bold text-zinc-300 text-center`}>
      ...
      <div className={`grid grid-cols-7 ${compact ? "gap-1" : "gap-2"} text-center text-xs font-bold uppercase tracking-wider text-zinc-500`}>
      ...
      <div className={`grid grid-cols-7 ${compact ? "gap-1" : "gap-2"}`}>
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              <p className={`${compact ? "text-xs" : "text-sm"} font-bold ${isToday ? "text-amber-400" : "text-zinc-300"}`}>{format(day, "d")}</p>
              {dayTodos.length > 0 && (
                <>
                  <p className={`${compact ? "text-[11px] mt-0.5" : "text-xs mt-1"} font-semibold text-zinc-300 leading-tight truncate`} title={firstTitle}>
                    {firstTitle}
                  </p>
                  {dayTodos.length > 1 && (
                    <p className={`${compact ? "text-[11px]" : "text-xs"} text-zinc-500 leading-tight font-medium`}>+{dayTodos.length - 1} more</p>
                  )}
                </>
              )}
            </button>
      ...
      <div className={`rounded-xl border border-zinc-800 bg-zinc-900/90 ${compact ? "p-3" : "p-4"}`}>
        <p className={`${compact ? "text-sm" : "text-base"} font-bold text-zinc-200 mb-2`}>
          ...
          <div className={compact ? "space-y-1.5" : "space-y-2"}>
            {selectedTodos.map((t) => (
              <div key={t._id} className="rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2">
                <p className={compact ? "text-sm font-semibold text-zinc-200" : "text-base font-semibold text-zinc-200"}>{t.title}</p>
              </div>
            ))}
          </div>
        ) : (
          <p className={compact ? "text-sm font-medium text-zinc-500" : "text-base font-medium text-zinc-500"}>No due tasks for this day.</p>
        )}
      </div>
    </div>
  );
